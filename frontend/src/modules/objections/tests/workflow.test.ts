...
test("повестка дня выбирает шаблон по виду обращения", () => {
  ...
  assert.match(
    agendaItemText(h.c),
    /касательно действия\/бездействия ДВГА по Атырауской области на аудиторский отчет от 30\.08\.2026 №Д-01/,
  );
  ...
  assert.match(
    agendaItemText(h.c),
    /на предписание ДВГА по Атырауской области от 30\.08\.2026 №Д-01 по аудиторскому отчету №АО-33 от 28\.08\.2026/,
  );
  ...
  assert.match(
    agendaItemText(h.c),
    /на предписание ДВГА по Атырауской области от 30\.08\.2026 №Д-01 по профилактическому контролю №ПК-33 от 27\.08\.2026/,
  );

  h.c.agendaDetails = { decisionKind: "quality-control" };
  assert.match(
    agendaItemText(h.c),
    /на результатам контроля качества ДВГА по Атырауской области от 30\.08\.2026 №Д-01/,
  );

  h.c.appealType = "Жалоба на акт о результате профилактического контроля";
  assert.match(
    agendaItemText(h.c),
    /на акт о результате профилактического контроля ДВГА по Атырауской области от 30\.08\.2026 №Д-01/,
  );
